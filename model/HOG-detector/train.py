...
def train_svm(train_data, label):
    ...
    # label (n,) --> response (n, 1)
    response = np.reshape(label, [-1, 1])
...
def main():
    ...
    # Save HOG feature dataset
    with open(args.output, 'wb') as f:
        pickle.dump([train_data, label], f)

    train_svm(train_data, label)


def load_and_train():
    with open('./data/HOG_dataset/train/training_data.pkl', 'rb') as f:
        train_data, label = pickle.load(f)
    train_svm(train_data, label)
